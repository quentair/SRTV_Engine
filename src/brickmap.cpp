...
uint8_t Brickmap::posToIndex(uint8_t x, uint8_t y, uint8_t z) const
{
	...
	// return the index inside the bucket of the voxel at given local position (so range between 0 and 31)
...
uint8_t Brickmap::getSubmaskIndex(uint8_t x, uint8_t y, uint8_t z) const
{
	...
	// return the index of the the bucket where the voxel at given local position is (so range between 0 and N_SUMBASKS)
	return ((x + z * BRICKMAP_SIZE + y * BRICKMAP_SIZE * BRICKMAP_SIZE) / (N_BITS_UINT32));
}
...
{
	// return the presence submask of 1 voxel at given local position
...
{
	// return the presence submask where the voxel at given local position should belong to
...
{
	// add the voxel presence submask of 1 voxel at given local position into its associated submasks 
